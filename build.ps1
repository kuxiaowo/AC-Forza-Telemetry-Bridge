...
& $PythonExe -m PyInstaller --clean --noconfirm --windowed --onedir --name ACForzaBridge --paths $sourceRoot --add-data "$sourceRoot\help.html;." --add-binary "$envRoot\Library\bin\tcl86t.dll;." --add-binary "$envRoot\Library\bin\tk86t.dll;." --distpath (Join-Path $buildRoot 'dist') --workpath (Join-Path $buildRoot 'build') --specpath $buildRoot (Join-Path $sourceRoot 'main.py')
